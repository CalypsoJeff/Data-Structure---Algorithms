...
class Trie{
    ...
    wordsStartingWithPrefix(prefix){
        let words=[]
        let curr = this.root;
        for(let i=0;i<prefix.length;i++){
            let charToFind = prefix[i];
        ...
        this._findAllWords(curr, prefix, words);
        return words;
    }
    _findAllWords(node, prefix, words) {
        ...
        for (let char in node.children) {
            this._findAllWords(node.children[char], prefix + char, words);
        }
    }
    displayTrie(){
...
newTrie.displayTrie()
console.log(newTrie.contains("banana"));//true
...
console.log(newTrie.wordsStartingWithPrefix("bana"));//true
